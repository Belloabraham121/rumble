...
This document **implements §1** of `BACKEND_API_ROADMAP.md`: concrete choices and how they map to code (`lib/rombo/*`).

---

## 1. Auth source of truth & Privy bridge

**Decision (phase 1 — current):** Rombo remains **`SessionUser`** via HTTP-only cookie (`lib/auth/session.ts`, email login). This stays the **source of truth for dashboard access**.
...
1. User signs in with existing Rombo auth → `getSession()` returns `{ email }`.
...
3. Store mapping **`rombo_user_id` ↔ `privy_user_id`** in **MongoDB** (see §5).
...
## 2. Wallet model per Rombo agent
...
**Configuration:** Set `ROMBO_AGENT_WALLET_MODEL` (see `.env.example`). Parsed in `lib/rombo/wallet-model.ts`.
...
|-------------|------------------|------------------------------|
| `ROMBO_TARGET_NETWORK=testnet` (default) | `84532` | `base-sepolia` |
| Override | `ROMBO_DEFAULT_CHAIN_ID` | Must match `CHAIN_ID_BY_SLUG` in `lib/rombo/chain-config.ts` |
...
**Introspection:** Server code can call `getRomboServerEnv()` (`lib/rombo/server-env.ts`) for booleans `hasMongo`, `hasPrivyApp`, `hasPrivyWalletAuthz`, `hasUniswap` without logging secrets.
