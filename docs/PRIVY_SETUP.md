# Privy — Rombo server setup

Use this with **`docs/BACKEND_API_ROADMAP.md` §2** and **`docs/ARCHITECTURE_DECISIONS.md`**. Rombo keeps **cookie sessions** as the dashboard auth source of truth in phase 1; Privy users are created or linked **after** login so embedded wallets map to your app user id.
...
3. Under **Authentication**, enable the providers you want (email, wallet, etc.) so they match how Rombo users sign in today or how you plan to migrate.
...
Runtime flags (no secret values) are exposed via `getRomboServerEnv()` in `lib/rombo/server-env.ts` (`hasPrivyApp`, `hasPrivyWalletAuthz`).
...
If you implemented server-side signing elsewhere, mirror the same **`AuthorizationContext`** construction and call **`generateAuthorizationSignature`** (or the SDK helper on `client.utils()`) before hitting Privy wallet endpoints. Rombo does not bundle that external project; paste or symlink the signing snippet here when you have a path on disk.

## 5. Implemented server flows (Rombo)
